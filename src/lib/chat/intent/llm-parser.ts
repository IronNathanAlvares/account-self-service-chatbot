...
export class LlmIntentParser implements IntentParser {
  ...
  async parse(message: string, context?: ParseContext): Promise<ParsedIntent> {
    ...
    const now = new Date();
    const dateContext = `\n\nCurrent date and time is ${now.toISOString()} (${now.toLocaleDateString("en-IE", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}). Resolve relative dates/times (today, tomorrow, next Tuesday, the 1st of next month, 4pm) against this and output future ISO 8601 values.`;
